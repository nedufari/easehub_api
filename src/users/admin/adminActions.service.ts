...
import { AdminRepository, EventDetailsRepsository } from './admin.repository';
import {
  IAdmin,
...
export class AdminActionService {
  constructor(
    ...
    private readonly eventdetailsripo: EventDetailsRepsository,
  ...
  async moshood(
    id: string,
    eventdto: PrepareSystemDto,
    
  ): Promise<{ message: string; eventdetails:EventDetailsEntity,filedownloadPath:string,guestPortalLink:string}> {
    try {
      ...
      const eventdetails = new EventDetailsEntity();
      eventdetails.event_location = eventdto.event_location;
      eventdetails.event_venue =eventdto.event_venue
      eventdetails.event_time = eventdto.event_time;
      eventdetails.event_title = eventdto.event_title;
      eventdetails.table_amount = eventdto.table_amount;
      eventdetails.registration_date = new Date();
      eventdetails.created_by = admin.name;
      (eventdetails.expected_number_of_guest =
        eventdto.expected_number_of_guest),
        (eventdetails.number_of_vendors = eventdto.number_of_vendors);

      if (
        eventdto.vendor &&
        eventdto.vendor.length === eventdto.number_of_vendors
      ) {
        eventdetails.vendors = eventdto.vendor.map((vendor) => ({
          call_time: vendor.call_time,
          number_of_staff: vendor.number_of_staff,
          brief_note: vendor.brief_note,
          department: vendor.department,
        }));
      } else {
        // Handle error: The number of vendors in the array does not match the expected number
        throw new HttpException(
          'The number of vendors in the array does not match the expected number.',
          HttpStatus.BAD_REQUEST,
        );
      }
      const savedEventDetails= await this.eventdetailsripo.save(eventdetails);
  

        // Retrieve the ID of the created event
    const createdEventId = savedEventDetails.id;

    // Generate the link for the guest portal
    const guestPortalLink = `http://localhost:3005/v1/api/guests/register-guest?eventdetailsid=${createdEventId}`;

      //download the old guest list 
      const guests = await this.guestripo.find();
      ...
      return {
        message:
          'you have successfully cleared the entire guest list and have also prepared the systen for a new event.',
        
        eventdetails:eventdetails,
        filedownloadPath: outputFilePath,
